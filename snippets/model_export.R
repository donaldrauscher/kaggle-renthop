...
step_output_name <- get_step_output(args$pipeline, args$step, step_def)
step_output_name_hashed <- get_step_output_hashed(args$pipeline, args$step, step_def)
step_outputs <- list(model = get(paste0(step_def$model, "_base")), validate_multiloss = validate_multiloss, validate_predictions = validate_predictions, test_predictions = test_predictions)
